...
if __name__ == '__main__':
    ...
    parser.add_argument("--mevis_pred_path", type=str, default=None)
    ...
    parent_dir = os.path.dirname(args.mevis_pred_path)
    sava_file = osp.join(parent_dir, 'davis_result.json')
    results = {
        'J'  : round(100 * float(np.mean(j)), 2),
        'F'  : round(100 * float(np.mean(f)), 2),
        'J&F': round(100 * float((np.mean(j) + np.mean(f)) / 2), 2),
    }
    with open(sava_file, 'w') as f:
        json.dump(results, f, indent=4)
    try:
        shutil.rmtree(args.mevis_pred_path)
        print(f"[INFO] Deleted prediction folder: {args.mevis_pred_path}")
    except Exception as e:
        print(f"[WARN] Failed to delete folder {args.mevis_pred_path}: {e}")
    end_time = time.time()
