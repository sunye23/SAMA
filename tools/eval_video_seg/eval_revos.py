...
if __name__ == '__main__':
    ...
    parser.add_argument("--visa_pred_path", type=str, default=None)
    ...
    parent_dir = os.path.dirname(args.visa_pred_path)
    sava_file = osp.join(parent_dir, 'revos_result.json')
    with open(sava_file, 'w') as f:
        json.dump(results, f, indent=4)
    try:
        shutil.rmtree(args.visa_pred_path)
        print(f"[INFO] Deleted prediction folder: {args.visa_pred_path}")
    except Exception as e:
        print(f"[WARN] Failed to delete folder {args.visa_pred_path}: {e}")
    data4csv = {}
