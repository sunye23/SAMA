...
def load_multi_part_safetensors(model_dir):
    state_dict = {}
    model_parts = sorted(glob.glob(os.path.join(model_dir, '*.safetensors')))

    for part_file in model_parts:
        print(f"Loading {part_file}...")
        part_state_dict = safetensors.torch.load_file(part_file, device="cpu")
        state_dict.update(part_state_dict)

    return state_dict

def rename_keys_in_pretrained(pretrained_state_dict: dict) -> dict:
    rename_map = {}
    for old_key in list(pretrained_state_dict.keys()):
        new_key = old_key

        if old_key.startswith("vision_model") or old_key.startswith("language_model") or old_key.startswith(
                "mlp1") or old_key.startswith("grounding_encoder.") or old_key.startswith(
                "text_hidden_fcs.") or 'g_weight' in old_key:
            new_key = "mllm.model." + old_key
        if new_key != old_key:
            rename_map[old_key] = new_key

    for old_k, new_k in rename_map.items():
        if 'grounding_encoder.' in old_k or 'text_hidden_fcs' in old_k or '.g_weight' in old_k or 'gamma' in old_key:
            pretrained_state_dict[new_k] = pretrained_state_dict[old_k]
            if '.g_weight' in old_k:
                pretrained_state_dict[old_k.replace('.g_weight', '.gamma')] = pretrained_state_dict[old_k]
        else:
            pretrained_state_dict[new_k] = pretrained_state_dict.pop(old_k)
...
class VideoLLaVASAMModel(LisaModel):
    def __init__(self,
        ...
        print(f'Load pretrained weight from {pretrained_pth}')
        if pretrained_pth is not None and pretrained_pth.endswith('.pth'):
            pretrained_state_dict = guess_load_checkpoint(pretrained_pth)
            self.load_state_dict(pretrained_state_dict, strict=False)
        elif pretrained_pth is not None and 'Sa2VA' in pretrained_pth:
            pretrained_state_dict = load_multi_part_safetensors(pretrained_pth)
            pretrained_state_dict = rename_keys_in_pretrained(pretrained_state_dict)
            self.load_state_dict(pretrained_state_dict, strict=False)
        else:
            print("pretrained_pth is wrong....")
            exit(0)
    ...
    def state_dict(self, *args, **kwargs):
        ...
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'Qformer' in k}
        )
        return to_return
