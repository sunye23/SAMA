...
class LLaVADataset(Dataset):
    ...
    def __init__(self,
        ...
        if repeat > 1:
            self.data = self.data * int(repeat)  # 物理复制
    ...
    def prepare_data(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            token_dict = self.get_inputid_labels(data_dict['conversations'], image_token_str, context_token_str=context_str)
            out_data_dict.update(token_dict)
        else:
            ...
            #     num_context_token = 32
            context_str = self.IMG_CONTEXT_TOKEN * num_context_token + '\n'
            token_dict = self.get_inputid_labels(data_dict['conversations'], None, context_str)
            out_data_dict.update(token_dict)
    ...
    def get_inputid_labels(self, conversations=None, image_token_str=None, context_token_str='') -> dict:
        input = ''
        out_conversation = []
        text_prompts = []
        question = None
        ...
        for index, msg in enumerate(conversations):
            if index == 0 and msg['from'] == 'human' and '<image>' not in msg['value'] and image_token_str is not None:
                msg['value'] = '<image>' + '\n' + msg['value']
            if msg['from'] == 'human' and index != 0 and '<image>' in msg['value']:
                msg['value'] = msg['value'].replace(DEFAULT_IMAGE_TOKEN, '').strip()

        for index, msg in enumerate(conversations):
            if msg['from'] == 'human':
                question = msg['value'].replace('<image>', '').strip()
                if image_token_str is None and '<image>' in msg['value']:
                    msg['value'] = msg['value'].replace('<image>', '')
                if index == 0:
                    if '<image>' in msg['value'] and image_token_str is not None:
                        msg['value'] = msg['value'].replace('<image>', '').replace('\n', '').strip()
                        msg['value'] = image_token_str + '\n' + context_token_str + msg['value']
                    else:
                        msg['value'] = msg['value'].replace('\n', '').strip()
                        msg['value'] = context_token_str + msg['value']
                input += msg['value'].strip()
            elif msg['from'] == 'gpt':
                text_prompts.append(question)
                conv_manager.add_conversation(question=question, answer=msg['value'].strip())
                if index - 1 != 0:
                    input = context_token_str + input
                out_conversation.append({
                    'input': input,
                    'output': msg['value'].strip()
                })
        ...
        if len(input_ids) > self.max_length:
            ...
            print_log(
                ...
                f'is longer than max_length, cut to {self.max_length}',
                logger='current')
        return {'input_ids': input_ids, 'labels': labels, 'prompts':text_prompts}
