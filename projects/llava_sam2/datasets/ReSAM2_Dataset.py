...
class VideoSAM2Dataset(Dataset):
    ...
    def dataset_map_fn(self, objects_expression_infos, n_frames, n_fast_frames=0):
        ...
        ret = {'conversation': text_dict['conversation'], 'prompts':text_dict['prompts']}
        return ret
    ...
    def prepare_text(self, n_frames, expressions, num_image_tokens=256, n_fast_frames=0):
        ...
        for i, exp in enumerate(expressions):
            ...
            text_prompt.append(questions[-1])
        ...
        return {'conversation': conversation, 'prompts':text_prompt}

    def remove_first_path_component(self, path):
    ...
    def __getitem__(self, index):
        ...
        video_path = os.path.join(self.sam2_folder, self.remove_first_path_component(expression_dict['video_path']))
        anno_path = os.path.join(self.sam2_folder, self.remove_first_path_component(expression_dict['anno_path']))
        ...
        sparse_indices = get_sparse_indices_uniform(n_frames, max_frames=self.max_frames)
        self.sparse_len = len(sparse_indices)
        dense_indices = get_dense_indices(len(sparse_indices), self.dense_len)
        selected_sparse = [sparse_indices[i] for i in dense_indices]

        dense_video_frames = [video_frames[_idx] for _idx in selected_sparse]
        sparse_video_frames = [video_frames[_idx] for _idx in sparse_indices]
        masklents = [masklents[_idx] for _idx in selected_sparse]

        data_dict = self.dataset_map_fn(objects_expression_infos, len(dense_video_frames), n_fast_frames=self.n_fast_images)
        ...
        pixel_values = []
        extra_pixel_values = []
        for index, frame in enumerate(sparse_video_frames):
            frame = frame[:, :, ::-1]
