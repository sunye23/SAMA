...
def refcocog_preprocess(example):
    ...
    return example
from projects.llava_sam2.datasets.utils import ConversationManager


def glamm_refcocog_map_fn(example, max_conv_len):
    conv_manager = ConversationManager(max_length=max_conv_len)
    # example {'id': str, 'refs': [{"setence", 'bbox', 'segmentation'},], 'img_file_name': str, 'caption': str}
    text_prompts = []
    question = None
    answer = None
    ...
    for index, msg in enumerate(messages):
        if msg['from'] == 'human':
            question = msg['value'].replace(DEFAULT_IMAGE_TOKEN, '').strip()
            ...
            # else:
            #     text_prompts.append(question)
                # text_prompts.append(conv_manager.merge_conversations() + "Answer the Question from the USER: " + question)
            conv_manager.add_conversation(question=question, answer=msg['value'])
            conversation.append({'input': input, 'output': msg['value']})
            input = ''
        else:
            raise NotImplementedError
    example.update({'conversation': conversation,'prompts':text_prompts})
    return example
...
def glamm_granf_map_fn(example):
    ...
    for msg in messages:
        if msg['from'] == 'human':
            ...
            text_prompts.append(input.replace(DEFAULT_IMAGE_TOKEN, '').strip())
            conversation.append({'input': input, 'output': msg['value']})
            input = ''
        else:
            raise NotImplementedError
