...
class ReferSegmDataset(RefCocoDataset):
    ...
    def prepare_data(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            token_dict = self.get_inputid_labels(conversations=data_dict['conversations'], image_token_str=image_token_str, context_token_str=context_str)
            out_data_dict.update(token_dict)
        else:
            token_dict = self.get_inputid_labels(conversations=data_dict['conversations'], image_token_str=None, context_token_str=None)
            out_data_dict.update(token_dict)
            out_data_dict['pixel_values'] = torch.zeros(1, 3, self.image_size, self.image_size)
        return out_data_dict

    def get_inputid_labels(self, conversations=None, image_token_str=None, context_token_str=None) -> dict:
        input = ''
        out_conversation = []
        text_prompts = []
        ...
        for index, msg in enumerate(conversations):
            if msg['from'] == 'human':
                ...
                if '<image>' in msg['value']:
                    ...
                    msg['value'] = image_token_str + context_token_str + msg['value']
                input += msg['value'].strip()
            elif msg['from'] == 'gpt':
                if index - 1 != 0 and image_token_str is not None:
