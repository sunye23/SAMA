...
from projects.llava_sam2.datasets.utils import get_sparse_indices, get_dense_indices, ConversationManager, get_sparse_indices_uniform
...
class VideoReVOSDataset(Dataset):
    ...
    def dataset_map_fn(self, data_dict, select_k=5):
        ...
        sparse_indices = get_sparse_indices_uniform(len_frames, self.max_frames)
        ...
        ret = {'images': images, 'video_masks': video_masks, 'conversation': text_dict['conversation'], 'prompts': text_dict['prompts'],
               'fast_images': fast_video_frames, 'fast_video_masks': fast_video_masks, 'dense_indices': dense_indices}
        return ret
    ...
    def prepare_text(self, n_frames, expressions, num_image_tokens=256, n_fast_images=50):
        ...
        for i, exp in enumerate(expressions):
            ...
            text_prompts.append(questions[-1])
        ...
        for i, (question, answer) in enumerate(zip(questions, answers)):
            if i == 0:
                ...
                qa_list.append(
                    {'from': 'human', 'value': frame_tokens + context_str + question + after_question_str}
                )
            else:
                qa_list.append(
                    {'from': 'human', 'value': context_str + question + after_question_str}
                )
            qa_list.append(
    ...
    def __getitem__(self, index):
        ...
        data_dict = self.dataset_map_fn(video_objects_infos, select_k=self.dense_len)
