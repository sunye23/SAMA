...
from projects.llava_sam2.datasets.utils import get_sparse_indices, get_dense_indices, ConversationManager
import random
import math
...
def _get_rawvideo_dec(video_path, select_frames=5):
    ...
    if num_frames <= select_frames:
        num_repeat = select_frames // num_frames
        num_sample = select_frames % num_frames
        sample_pos = list(range(num_frames)) * num_repeat
        if num_sample > 0:
            sample_pos += uniform_sample(num_frames, num_sample)
...
class VideoChatUniViDataset(Dataset):
    ...
    def dataset_map_fn(self, data_dict, select_k=5):
        ...
        images = _get_rawvideo_dec(video_file, select_frames=self.sparse_len)
        split_point = np.linspace(0, self.sparse_len, num=self.dense_len + 1, dtype=int)
        dense_indices = [
            np.random.randint(split_point[i], split_point[i + 1])
            for i in range(self.dense_len)
        ]
        # dense_indices = get_dense_indices(self.sparse_len, self.dense_len)
        self.dense_len = len(dense_indices)
        if self.use_fast:
        ...
        if self.use_fast:
            text_dict = self.prepare_text(
                self.dense_len, conversation, num_image_tokens=self.patch_token,
        ...
        ret = {'images': images,'prompts': text_dict['prompts'],'conversation': text_dict['conversation'], 'fast_images': fast_images, 'dense_indices': dense_indices}
        return ret
    ...
    def prepare_text(self, n_frames, conversation, num_image_tokens=256, n_fast_images=0):
        ...
        for i, (question, answer) in enumerate(zip(questions, answers)):
            text_prompts.append(question.replace('<image>', '').strip())
            if i == 0:
                # text_prompts.append("Answer the Question from the USER: "+temp_question)
                ...
                qa_list.append(
                    {'from': 'human', 'value': frame_tokens + context_str + question}
                )
            else:
                # text_prompts.append(conv_manager.merge_conversations() + "Answer the Question from the USER: " + temp_question)
                qa_list.append(
                    {'from': 'human', 'value': context_str + question}
                )

            # conv_manager.add_conversation(question=temp_question, answer=answer)
            qa_list.append(
        ...
        conversation = []
        temp_question = None
        for index, msg in enumerate(qa_list):
            if msg['from'] == 'human':
                input += msg['value']
            elif msg['from'] == 'gpt':
                temp_question = input.replace('<image>', '').strip()
                conversation.append({'input': input, 'output': msg['value']})
                conv_manager.add_conversation(question=temp_question, answer=msg['value'])
                input = ''
            else:
                raise NotImplementedError
        ...
        return {'conversation': conversation, 'prompts':text_prompts}

    def __getitem__(self, index):
        index = index % self.real_len()
        selected_data_dict = copy.deepcopy(self.text_data[index])
        data_dict = self.dataset_map_fn(selected_data_dict, select_k=self.dense_len)
