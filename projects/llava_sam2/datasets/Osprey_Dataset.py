...
class OspreyDataset(Dataset):
    ...
    def _process_conversation(self, converations, n_regions, region_pixels):
        ...
        for index, msg in enumerate(messages):
            if index == 0 and msg['from'] == 'human' and '<image>' not in msg['value']:
                msg['value'] = '<image>' + '\n' + msg['value']
            if msg['from'] == 'human' and index != 0:
        ...
        return conversation, text_prompts, start_region_str
    ...
    def dataset_map_fn(self, data_dict):
        ...
        conversations, text_prompts, start_region_str = self._process_conversation(conversations, len(masks), region_pixels)
        _ret['conversation'] = conversations
        _ret['prompt_masks'] = masks
        _ret['prompts'] = text_prompts
        return _ret, start_region_str
    ...
    def __getitem__(self, index):
        ...
        # parse datasets
        result, start_region_str = self.dataset_map_fn(data_dict) # {'image', 'height', 'width', 'conversation', 'masks'}
        ...
        temp_conversations = data_dict['conversation']
        for idx, item in enumerate(temp_conversations):
            if idx == 0 and DEFAULT_IMAGE_TOKEN in item['input']:
                item['input'] = item['input'].replace(DEFAULT_IMAGE_TOKEN, '').replace('\n', '').strip()
                item['input'] = image_token_str + '\n' + start_region_str + context_str + item['input']
            else:
                item['input'] = context_str + item['input']
...
class OspreyDescriptionDataset(OspreyDataset):
    ...
    def __init__(self,
        ...
        super(OspreyDescriptionDataset, self).__init__(
            ...
            num_temporal_token=num_temporal_token,
            num_spatial_token=num_spatial_token,
    ...
    def dataset_map_fn(self, data_dict):
        ...
        conversations, text_prompts, start_region_str = self._process_conversation(descriptions, len(masks), region_pixels)
        _ret['conversation'] = conversations
        _ret['prompt_masks'] = masks
        _ret['prompts'] = text_prompts
        _ret['start_region_str'] = start_region_str
        return _ret, start_region_str
