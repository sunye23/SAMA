...
class GCGVideoInferenceDataset(Dataset):
    ...
    VP_START_TOKEN = '<vp>'
    VP_END_TOKEN = '</vp>'
    def __init__(self, video_folder, data_json, qa_per_item=5):
        """
        参数:
            ...
            for i in range(num_splits):
                ...
                for idx_conv in range(current_round):
                    ...
                    if num_seg != valid_seg:
                        filter_num = filter_num + 1
                        continue
                    system_prompt = 'You are giving five frames sampled from a video. The first frame shows several colored boxes, each highlighting a interested object.' \
                                    ' You need to answer the question about the interested objects based on the video content. ' \
                                    'Answer without mentioning any box or color words as if observing the video in real-time. **Provide the coordinates [x0,y0,x1,y1] for every objects mentioned in the answer**. Response in JSON format like: {"answer":"The person [2,1,321,971] is bowling, rolling the bowling ball [582,553,643,614] down the lane towards the pins."}. ' \
                                    'Question: '
                    if num_seg > 0 and 'with interleaved segmentation masks.' not in question['value']:
                        question['value'] = system_prompt + question['value']
                        # question['value'] = question['value'] + ' Please respond with interleaved segmentation masks for the corresponding parts of the answer.' 
    ...
    def __getitem__(self, index):
        ...
        data_dict['conversation'] = record['conversations']
        ...
        # __getitem__ 内采样行 —— 直接替换
        frames_files = [frames_files[i] for i in sample_five_indices(len(frames_files))]
        ...
        for item in obj_list:
            ...
            for sub_item in item:
                ...
                prompt_masks.append(mask_info[temp_sub_item]['mask'])
        data_dict['text_prompts'] = None
        ...
        obj_colors = []
        # 如果没有mask 跳过处理
        for mask_item in prompt_masks:
            if len(mask_item) == 0:
                continue

            # 取第一帧对应的 mask
            single_mask = mask_item[0]  # shape = (H, W)，numpy数组
            ...
            if single_mask.sum() == 0:
                ...
                min_row = int(nonzero[:, 0].min().item())
                max_row = int(nonzero[:, 0].max().item())
                min_col = int(nonzero[:, 1].min().item())
        ...
        #
        # # 可以将绘制完的图像放回 data_dict 里，或另存为 data_dict['image_with_box']
        # image_draw.save('/Sa2VA/reproduce/groundingLMM/test.png')
        # print("obj_colors is: {}".format(obj_colors)) as
        # 将对象对应的颜色信息写入 data_dict，方便后续使用
        data_dict['obj_colors'] = obj_colors
        ...
        # if '<image>' not in processed_question:
...
def main():
    ...
    json_file = '/Sa2VA/reproduce/Sa2VA/video_region_meta/BenchMark_merged_final_with_qid.json'
    dataset = GCGVideoInferenceDataset(video_folder, json_file, qa_per_item=1)
    ...
    index = 0
    output_base_dir = '/Sa2VA/reproduce/Benchmark/evaluation/sa2va_prediction/video_gcg/gemini_1_5_pro_coordinates'
    os.makedirs(output_base_dir, exist_ok=True)
    for data_batch in tqdm(dataloader):
        instruction = data_batch['processed_question']  # ✅ 正确，data_batch 才是 batch
        images = data_batch['images']
        groundtruth = data_batch['groundtruth']
        file_name = data_batch['file_name']
        frames = data_batch['frames']
        record_id = data_batch['record_id']
        prompts = []
