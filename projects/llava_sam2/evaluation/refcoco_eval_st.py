...
IMAGE_FOLDER = '/datasets/datasets_sysy/llava_all_image_video/coco/train2014/'
DATA_PATH = '/datasets/datasets_sysy/llava_all_image_video/coco/'
...
def main():
    ...
    for idx in tqdm.tqdm(per_rank_ids):
        ...
        for text in texts:
            ...
            pred_mask = model.predict_forward(**_data_batch, tokenizer=tokenizer, prediction_only=False)['prediction_masks']
            if len(pred_mask) == 0:
                # give a zero mask
                print("No seg pred !!!")
                pred_masks.append(None)
            else:
                _ret_mask = pred_mask[0]  # 已经是 numpy.ndarray
